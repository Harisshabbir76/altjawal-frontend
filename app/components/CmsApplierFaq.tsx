...
export default function CmsApplierFaq() {
  ...
  useEffect(() => {
    ...
    if (bgImg && blocksMap['faq-hero-bg']) applyImage(bgImg, blocksMap['faq-hero-bg']);

    const frameImg = document.querySelector('.faq-hero__frame-img') as HTMLElement | null;
    if (frameImg && blocksMap['faq-frame-img']) applyImage(frameImg, blocksMap['faq-frame-img']);

    const paperBg = document.querySelector('.faq-contact__paper-bg') as HTMLElement | null;
    if (paperBg && blocksMap['faq-paper-bg']) applyImage(paperBg, blocksMap['faq-paper-bg']);
  }, [blocksMap, lang]);
